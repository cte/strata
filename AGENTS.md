...
**UI primitives are Base UI, not Radix.** All `apps/web/src/components/ui/` primitives are built on `@base-ui/react` (see [ADR-0004](docs/adr/0004-base-ui-over-radix.md)). `@radix-ui/*` and `cmdk` have been removed — do not reintroduce them or copy from the classic Radix shadcn registry. When adding a component from shadcn, use the **Base UI** registry variant (e.g. `https://ui.shadcn.com/docs/components/base/<name>`). Base UI conventions:

- **No `asChild`.** Compose with the `render` prop (`<Dialog.Trigger render={<Button />} />`) and `useRender` for custom wrappers. There is no Radix `Slot`. `Button` exposes a `render` prop and forwards `ref`; the sidebar uses a local `renderSlot` (`useRender`) helper. When porting `<Trigger asChild>{child}`, pass the **existing** child to `render` (`render={child}`) — do not wrap it in a new `<span>`/`<div>`, which reintroduces an unconstrained DOM node that can defeat a `min-w-0`/`truncate` layout.
- **State styling uses Base UI data attributes:** `data-open`/`data-closed`, `data-checked`/`data-unchecked`, `data-selected`, `data-highlighted`, `data-panel-open`, `data-starting-style`/`data-ending-style` — not Radix's `data-[state=open]` or cmdk's `[cmdk-*]` selectors, and not `focus:` for menu/list highlight (use `data-highlighted:`). These bare `data-*` attributes are **not** built-in Tailwind v4 variants, so they are registered as `@custom-variant`s in `globals.css`; without that registration `data-open:`/`data-highlighted:` utilities silently compile to nothing (dead highlight styling and dead open/close animations). Adding or editing a `@custom-variant` needs a dev-server restart (Tailwind caches its variant set; HMR alone won't regenerate the utilities).
- **Closing popups need `data-closed:fill-mode-forwards`.** `tw-animate`'s `animate-out` uses `animation-fill-mode: none`, so after the `exit` fade completes the element snaps back to base opacity for the frames before Base UI unmounts it — a visible flash on close. Every closing surface (`dialog`/`sheet` backdrop + popup, `dropdown-menu`, `popover`, `tooltip`, `hover-card`, `select`) must include `data-closed:fill-mode-forwards` so it holds its faded-out end state until unmount.
- **Popups are `Portal > Positioner > Popup`.** `side`/`align`/`sideOffset`/`collisionPadding` live on the `Positioner`; the popup's transform origin is `origin-[var(--transform-origin)]`; available space is `var(--available-height)`/`var(--available-width)`.
- **Put the stacking `z-50` on the `Positioner`, not the `Popup`.** The `Positioner` positions via a `transform`, which creates its own stacking context; a `z-50` on the inner `Popup` is then trapped inside it and loses to the app shell's `.root { z-index: 1; isolation: isolate }`, so the popup paints **behind** the page and its content becomes unclickable (clicks fall through to whatever is beneath). The `Positioner` is the top-level portaled element, so its z-index is what competes at the body level — set `className="z-50"` there for every menu/popover/tooltip/preview-card/select.
- **Event/handler shapes:** `onOpenChange(open, details)`, `onCheckedChange(checked, details)`, `onValueChange(value, details)`; tooltip/menu timing uses `delay`/`closeDelay` (not `delayDuration`). Menu items use `onClick` + `closeOnClick` (there is no Radix `onSelect`). Tooltips require a `TooltipProvider` ancestor.
- **Command palette is Base UI `Combobox`** (`ui/command.tsx`), which is data-driven: pass grouped/flat `items` to `Command` and render filtered results through `CommandList`'s render-function child + `CommandCollection`. For externally-filtered, children-driven lists (the ⌘K session palette) pass `filter={null}` with controlled `inputValue`/`onInputValueChange` and render plain children.
...
- Component directory split — keep these two kinds of component apart so it is always obvious which is which:
  - `@/components/ui` (`apps/web/src/components/ui/`) holds **only official shadcn/ui components** — added/updated from the shadcn **Base UI** registry (it is the `ui` alias in `components.json`). Treat these as vendored: prefer the shadcn CLI's Base UI variant, and avoid hand-authoring or dropping our own components here so registry updates stay clean. Never re-add the Radix variant.
