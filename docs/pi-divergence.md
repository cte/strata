...
| Tool prompt guidance        | `packages/tools/src/types.ts` `promptSnippet`/`promptGuidelines` + `packages/agent/src/runContext.ts` | `packages/coding-agent/src/core/tools/*` + `core/system-prompt.ts`          | Same pattern: active tools can carry one-line summaries and usage guidelines, and the run context injects them into the system prompt. This keeps edit/write discipline with the tool definition instead of scattering it through global prompt text. |
...
| Editor stays interactive while agent runs | `packages/tui/src/app/app.ts:runAgent` (no `editor.disabled` toggle) | Pi never disables the editor while streaming                                | Up/Down still browse history, typing still works. Plain Enter queues steering via the shared loop's mid-run drain point; Alt+Enter queues follow-up for the point where the agent would otherwise stop. |
| Streaming queues            | `packages/tui/src/app/app.ts:queueSteeringMessage` / `queueFollowUpMessage` + `state.steeringMessages` / `state.followUpMessages` | `interactive-mode.ts:session.prompt(..., { streamingBehavior })`, `handleFollowUp`, and `restoreQueuedMessagesToEditor` | Same pattern: Enter while streaming steers, Alt+Enter follows up, Alt+Up restores queued messages, and Escape restores queued text before aborting. |
...
| Ctrl+Z suspend/resume       | `packages/tui/src/app/app.ts:handleCtrlZ`                                     | `interactive-mode.ts:handleCtrlZ`                                           | Same suspend pattern: stop the TUI before `SIGTSTP`, keep the event loop alive, temporarily ignore `SIGINT`, then restart and force-redraw on `SIGCONT`. |
| Auto-compaction              | `packages/agent/src/agentLoop.ts` + `packages/agent/src/compaction.ts:shouldAutoCompact` | Pi auto-compacts in `AgentSession` when context fills past `contextWindow - reserveTokens` or a provider reports context overflow | Same reserve-token threshold and shared-loop ownership. Strata compacts before continued turns and after successful final-answer turns; detected context-overflow errors compact and retry once instead of using the transient-error retry path. |
| Compaction (incremental)     | `packages/agent/src/compaction.ts`                                           | `packages/coding-agent/src/core/compaction/compaction.ts`                   | Same prompts (verbatim), incremental-update flow, keep-recent cut points, split-turn prefix summaries, and append-only compaction checkpoints. Pi additionally tracks read/modified file operations and supports extension-provided compaction content. |
...
| `edit`| `fs.edit` | Multi-edit array supported — `edits: [{oldText, newText}, ...]` matched against the original file with overlap detection. Scalar `oldText`/`newText` still works for legacy/direct calls but is no longer advertised in the model-facing schema, matching Pi. Pi-style matching now preserves leading/trailing edit text exactly, normalizes CRLF/BOM safely, supports fuzzy matching for common model text differences, and returns unified diff hunks in the result. |
...
| **Compaction metadata/hooks** | Append-only SQLite `compaction.completed` events with summary/checkpoint boundaries | Append-only JSONL compaction entries with optional details and extension hooks | Core context behavior now matches Pi's trigger ownership, reserve-token threshold, keep-recent/split-turn compaction, and overflow retry. Strata does not yet track file-operation details or support extension-provided compaction content. |
