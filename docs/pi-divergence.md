...
user cancels. Cortex now matches: `runAgentLoopEvents` is a `while (true)`
...
| Auto-compaction              | `packages/agent/src/compaction.ts:shouldAutoCompact` (75% threshold)         | Pi auto-compacts in-loop when context fills past a threshold                | Same threshold, slightly different timing — cortex runs auto-compact between turns rather than mid-iteration.                                       |
...
defaults now match pi where it makes sense; cortex-specific safety (repo
scope, `wiki/raw/` exclusion, symlink rejection) is preserved.

| Pi    | Cortex     | Status                                                                                                                                              |
| ----- | ---------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ls`  | `fs.list`  | Default `limit` raised 200 → 500. Cortex adds `recursive` and `includeRaw` (cortex-specific). Empty-path coerced to `.`.                            |
| `read`| `fs.read`  | Added `offset` (1-indexed start line) + `limit` (line count). Result includes `firstLine` / `lastLine` / `totalLines`. Image content unsupported (cortex limitation). |
...
| `bash`| `shell.run`| Empty `cwd` coerced to `.`. Cortex retains explicit `cwd`/`shell`/`timeoutMs` args; pi's `bash` infers cwd.                                          |
...
| ----------------------------- | ------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Editor**                    | `packages/tui/src/editor.ts` (~300 lines, single-line prompt + history + autocomplete) | `packages/tui/src/components/editor.ts` (~3000 lines, multi-line cursor, undo, kill-ring, paste markers) | Cortex started simple and hasn't outgrown that yet. Big port deferred until users hit limits.                                                       |
...
## Cortex-only tools (no pi equivalent)

These tools have no counterpart in pi-mono. They exist because cortex's
...
- Cortex has four hand-rolled copies of `optionalString` / `optionalBoolean` / `optionalInteger` (in `args.ts`, `fsTools.ts`, `wikiTools.ts`, `shellTools.ts`). The empty-path-coercion fix happened at every site individually; consolidating to a single shared helper would prevent future drift.
