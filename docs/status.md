...
| Agent loop | Present, learning-context aware | `packages/agent/src/agentLoop.ts` runs bounded model/tool iterations, records sessions, records `file.changed` events when write tools mutate files, and uses `packages/agent/src/runContext.ts` to inject memory, active todos, and a compact skill index. The current run context is recorded as `message.system_context`. | Add automatic post-run reflection once the manual reflection flow has more mileage. |
...
The next milestone is the first scheduled maintenance slice:

1. Add a small job registry with named maintenance jobs and stable result records.
2. Add CLI entry points such as `cortex maintain run <job>` and `cortex maintain list`.
3. Implement initial jobs for wiki lint dry-run, stale todo/action review, memory review, skill inventory, and index refresh proposal checks.
4. Persist job runs as sessions/traces so maintenance is auditable and searchable.
5. Keep jobs manually triggerable first; add cron-style recurring execution after the job contracts are stable.

Completing this milestone lets Cortex maintain the brain content proactively instead of waiting for one-off prompts.
