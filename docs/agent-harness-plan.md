...
- `ToolDefinition.promptSnippet` and `promptGuidelines` are part of the model-facing tool contract. `ToolRegistry.list()` exposes normalized guidance for the active profile, and `buildRunContext()` injects it into the run's system context before the model request.
...
- Mid-run user steering follows Pi's queue model. `AgentRunConfig.getSteeringMessages` is polled after each assistant response plus its tool batch and injects returned user messages before the next model request. `AgentRunConfig.getFollowUpMessages` is polled only when the agent would otherwise stop, after tool calls and steering are exhausted. Queue draining is caller-owned, so TUI/web/RPC surfaces can choose one-at-a-time or all-at-once behavior without duplicating the loop.
