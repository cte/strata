...
2. A Bun/TypeScript agentic harness that can query, maintain, improve, and locally extend that wiki through explicit tools, persistent traces, memory, skills, Routine-triggered maintenance jobs, extensions, a TUI, a local browser chat UI, and a local web control plane for connector setup.
...
### 2. Action Schema and Future Extraction

Automated action extraction has been reset. The previous daily TODO extractor was removed after producing poor action ledgers. The clean starting point is the action-item schema and manual Markdown-backed management surface:

`wiki/actions/*.md -> @strata/core/wiki-actions -> wiki.actions.* -> /actions`

Future action intelligence should start by defining the action item data contract, write-back semantics, evidence model, and review lifecycle. Automated discovery should then be implemented through the general Routine primitive, with TODO candidates emitted as schema-valid Routine artifacts before any publication into `wiki/actions/`.

Detailed plan: [extraction-framework-plan.md](./extraction-framework-plan.md).

### 3. Routines

Routines are named, reusable, local automations that run agent sessions with explicit input schemas, output schemas, tool profiles, required skills, deterministic pre-run Jobs, triggers, and publication policies. They adapt the useful shape of Anthropic Claude Code routines while staying local-first and trace-backed.

Routines sit above low-level deterministic Jobs and trigger records:

`Routine definition -> routine.run Job -> shared agent loop -> schema-valid Routine artifacts`

The first validation use case is Granola daily TODO discovery: a scheduled routine pulls/indexes recent Granola notes, invokes a focused TODO extraction skill, and emits evidence-backed TODO candidates as artifacts for review.

Detailed plan: [routines-plan.md](./routines-plan.md).
...
- Native wiki retrieval tools, including quick page search plus chunk-level hybrid retrieval over the local SQLite retrieval index.
...
- Registered deterministic Jobs plus Routine triggers for recurring connector pulls, wiki hygiene, retrieval refreshes, Routine runs, and learning tasks.

Detailed plan: [agent-harness-plan.md](./agent-harness-plan.md).

### 5. Interactive Agent UI

The agent should be able to ask the user a bounded clarifying question through a native trace-backed primitive before the broader extension runtime exists. This should be implemented as a normal `user.ask` tool over a small shared agent UI adapter: TUI and web chat can provide interactive adapters, while CLI print and scheduled jobs return explicit unavailable/cancelled results instead of blocking indefinitely.

This primitive is intentionally narrower than Pi's full extension UI API, but should be shaped so future extensions can expose it as `ctx.ui.select()`, `ctx.ui.confirm()`, and `ctx.ui.input()`.

Detailed plan: [interactive-agent-ui-plan.md](./interactive-agent-ui-plan.md).
...
The web control plane is a future local browser UI for setup and operations, not a separate product and not the ingestion runtime itself. It should expose connector setup, OAuth/token status, dry-run pulls, trace-backed ingest activity history, retrieval-index management, Routine authoring/triggers, and review for sources such as Notion, Granola, and Slack.
...
- Native user interaction: human clarification should be available as a first-party trace-backed tool (`user.ask`) through a shared surface adapter, not hidden prompt text or a TUI-only special case. Non-interactive runs must fail closed with an explicit unavailable/cancelled result rather than hanging.
- Routines as durable automation contracts: reusable agent automations should be modeled as Routines with structured input/output schemas, tool profiles, skills, pre-run Jobs, triggers, artifacts, and explicit publication policies rather than as free-form scheduled prompts.
...
- Evidence-backed extraction: extracted TODOs, decisions, and similar artifacts should be traceable to source spans, routine/artifact versions, confidence, and rationale. Re-indexing historical wiki days must be resumable and idempotent.
...
- Scheduled upkeep: recurring local Routines should keep connectors, the wiki, memory, skills, and indexes healthy through the shared Routine trigger scheduler and deterministic Jobs.
...
7. Add recurring automation for connector pulls, raw-to-wiki indexing, wiki hygiene, memory review, skill curation, stale actions, and index refreshes. Status: `@strata/jobs` provides registered deterministic jobs, trace-backed `job` sessions, `routine.run`, a PM2 worker, and a scheduler over durable `routine_triggers`. The standalone Schedule concept and `agent.prompt` job have been removed; recurring automation is now modeled as a Routine with triggers, and the safe `wiki.hygiene` job runs entity-consolidation proposal generation plus curated-first retrieval-index refresh without directly rewriting wiki pages. Next deepening: add the Granola daily TODO Routine and its extraction skill.
8. Continue improving the TUI around the richer agent runtime.
9. Return to source ingestion and wiki automation once the harness can maintain the wiki with observable tools and learning loops. Status: Notion, Granola, and Slack raw snapshots now run through the shared connector runner; Slack also has initial checkpointed sync and Socket Mode tailing. Granola raw backfill now handles official cursor pagination/detail transcript fetches, `strata ingest granola propose` stages review proposals, and `strata ingest raw index --source all|granola|notion|slack` automatically creates curated meeting, project, source, people, decision, thread, index, and log pages from supported raw snapshots. Decision extraction is conservative and evidence-oriented. Automated action promotion is currently disabled. Workspace-specific classification vocabulary now loads from the local ingest taxonomy at `.strata/ingest/taxonomy.json` through `@strata/ingest/ingest-taxonomy`, so canonical project aliases and local Slack materiality/ignore patterns are no longer product-code constants. Raw-to-wiki item events now include structured classification reasons, and the unified `/review` inbox can confirm classification corrections immediately or review LLM-staged taxonomy schema proposals. Slack raw-to-wiki dedupes snapshots, filters low-signal threads, writes material Slack snapshots to `wiki/sources/slack/` instead of bulk `wiki/threads/`, and has been applied to the current local raw Slack corpus. `strata wiki archive-generated-slack-threads` archives legacy generated Slack thread pages out of `wiki/threads`, `strata wiki compact-index` keeps `wiki/index.md` human-scale, and `strata wiki search-index refresh` builds the local curated-first SQLite retrieval index over curated pages, source pages, and raw evidence. The index now includes chunk-level FTS rows, extracted wiki links, and the `wiki.retrieve` agent tool for local hybrid retrieval with lexical-vector scoring, reciprocal-rank fusion, graph expansion, and token-budgeted evidence packing. `wiki.entities` produces structured consolidation reports plus deduplicated pending proposals, emits guarded exact canonical merge patches for conservative small project duplicates, and falls back to manual review for ambiguous cases; `wiki.hygiene` makes entity proposal generation plus retrieval refresh available as a Routine pre-run/job step. `--index` hooks are present on Granola, Notion, and Slack pulls.
10. Rebuild action intelligence from the action-item schema and Routine artifacts. Status: reset. The previous generalized daily TODO extraction implementation, candidate store, CLI, web review queue, and raw-to-wiki action promotion were removed on 2026-05-29. `/actions` remains a manual Markdown-backed action manager over `wiki/actions/mine.md` and `wiki/actions/theirs.md`. Next work is to build the Routine primitive, then use a Granola daily TODO Routine as the first schema-valid, evidence-backed candidate producer before adding action review/write-back.
...
14. After connector contracts and automated source-to-wiki flows are stable, deepen the local web control plane for connector setup, dry-runs, Routine triggers, ingest history, saved defaults, taxonomy review, action management, retrieval-index management, and proposal review. Status: skeleton present with connector list, generic connector-run operations, Notion page snapshots, Granola key setup plus bounded one-off backfill, Slack status plus checkpointed one-off sync/backfill controls, Notion MCP auth, ingest activity, wiki action management backed by `wiki/actions/`, retrieval-index status/rebuild controls under `/index`, a unified `/review` inbox for classification corrections and proposals, guarded exact consolidation apply, Routine authoring/template/trigger controls over `@strata/routines` and `@strata/jobs`, non-secret connector config profiles exposed through CLI/tRPC/connector saved-default controls, and an experimental chat-side terminal panel now backed by a Strata-owned Ghostty/libghostty WASM artifact with a handwritten fallback.
...
16. Add a native interactive agent UI primitive before the full extension runtime. Status: planned in [interactive-agent-ui-plan.md](./interactive-agent-ui-plan.md); implement a `user.ask` tool plus shared UI adapter, TUI question component, web pending-question card/response path, non-interactive unavailable behavior, cancellation, and trace events.
17. Add Routine definitions, structured Routine artifacts, and a `routine.run` job. Status: partial. `@strata/routines` now provides Routine/RoutineRun/RoutineArtifact types, SQLite tables/migration, store helpers, input/output validation, prompt envelope construction, a per-run `routine.output.submit` tool, exports, built-in infrastructure templates, and tests. `@strata/jobs` registers `routine.run` and executes routines through the shared job runner and agent loop, including schema-valid output artifact persistence and required-output enforcement. CLI commands, web tRPC procedures, and the `/routines` browser UI expose routine definitions, templates, triggers, manual runs, run history, and artifacts. Next work is the Granola TODO routine.
18. Add a Pi-style local extension runtime after the core registry/tool-pack composition path is stable. Status: planned in [extensions-plan.md](./extensions-plan.md); no dynamic extension loader, public `ExtensionAPI`, extension command registry, extension hooks, or extension UI API exists yet.
...
- TUI interaction patterns, especially transcript display, prompt editing, auth flow, session selection, tool-call rendering, and bounded user-question dialogs.
...
- Routines as reusable scheduled or triggered agent workflows, adapted locally with structured input/output and artifact review.
...
- [extraction-framework-plan.md](./extraction-framework-plan.md): reset extraction architecture notes and schema-first action extraction redesign.
- [routines-plan.md](./routines-plan.md): local-first Routine definitions, triggers, structured outputs, artifacts, and the Granola daily TODO routine plan.
- [agent-harness-plan.md](./agent-harness-plan.md): model loop, tools, memory, skills, traces, and learning architecture.
- [interactive-agent-ui-plan.md](./interactive-agent-ui-plan.md): native `user.ask` tool and shared surface adapter for TUI/web human clarification.
