...
Routines sit above low-level Jobs and Schedules:
...
- Registered jobs and scheduled maintenance for recurring connector pulls, wiki hygiene, retrieval refreshes, Routine runs, scheduled agent prompt sessions, and learning tasks.
...
The web control plane is a future local browser UI for setup and operations, not a separate product and not the ingestion runtime itself. It should expose connector setup, OAuth/token status, dry-run pulls, trace-backed ingest activity history, retrieval-index management, schedules, and proposal review for sources such as Notion, Granola, and Slack.
...
17. Add Routine definitions, structured Routine artifacts, and a `routine.run` job. Status: partial. `@strata/routines` now provides Routine/RoutineRun/RoutineArtifact types, SQLite tables/migration, store helpers, input/output validation, prompt envelope construction, a per-run `routine.output.submit` tool, exports, and tests. `@strata/jobs` registers `routine.run` and executes routines through the shared job runner and agent loop, including schema-valid output artifact persistence and required-output enforcement. CLI commands and web tRPC procedures now expose routine definitions, manual runs, run history, and artifacts. Next work is the `/routines` browser UI and the Granola TODO routine.
