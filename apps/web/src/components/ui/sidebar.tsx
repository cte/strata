...
// Base UI composition helper replacing the Radix `asChild`/`Slot` pattern:
// when `render` is provided it becomes the rendered element, otherwise the
// `defaultTag` is used.
function renderSlot(params: {
  ...
  return useRender({
    render: params.render ?? React.createElement(params.defaultTag),
    ref: params.ref,
    props: params.props,
  });
}
  ...
  ({ className, ...props }, ref) => {
    return (
      <main
        ref={ref}
        className={cn(
          // Viewport-locked shell column: fixed height with internal scrolling
          // owned by each page, so full-bleed surfaces (chat) and scrollable
          // pages never need calc(100dvh - X) math or negative-margin breakouts.
          "relative flex h-dvh min-w-0 flex-1 flex-col overflow-hidden bg-bg",
...
>(
  (
    ...
    // Base UI tooltips have no `hidden` prop; skip rendering the tooltip when
    // the sidebar is expanded or on mobile (matching the old behavior).
