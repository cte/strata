...
describe("wiki actions", () => {
  ...
  test("adds source links and preserves extraction metadata when context changes", async () => {
    const repoRoot = await mkdtemp(path.join(os.tmpdir(), "strata-actions-"));
    try {
      const item = await addWikiAction(repoRoot, {
        owner: "mine",
        title: "Prepare the launch checklist",
        context: "Extracted from daily.todo.",
        source: {
          target: "raw/slack/2026-05-09-launch",
          label: "Launch thread",
        },
        metadata: {
          extractionCandidateId: "extracted_abc123",
          extractionRunId: "extract_session123",
          lineStart: 10,
        },
        now: new Date("2026-05-28T12:00:00.000Z"),
      });
      ...
      expect(file).toContain('"extractionCandidateId":"extracted_abc123"');
      expect(file).toContain('"extractionRunId":"extract_session123"');
